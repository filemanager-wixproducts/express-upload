...
var MultipartUploader = function(options) {
  ...
  // Add Region (This Prevents 307 Temporary Redirect Fails)
  aws.config.update(_.extend({ region: options.region }, options.credentials));
  this.s3 = new aws.S3();
};
...
MultipartUploader.prototype.config = function(options) {
  this.options = _.extend(this.options, options);
};

MultipartUploader.prototype.handleError = function(location, err) {
  this.emit('error', { e: location, data: err});
};
...
MultipartUploader.prototype.getObjectInfo = function(Bucket, Key, cb) {
  var self = this;
  this.s3.headObject({ Bucket: Bucket, Key: Key}, function(err, data) {
    if(err) { self.handleError('this.s3.headObject', err); cb(err, null); }
...
MultipartUploader.prototype.abortUploads = function(Bucket) {
  ...
  this.listUploads(Bucket, function(err, data) {
    if(err) { self.handleError('this.s3.listUploads', err); return; }
    ...
    if(data.length > 0) {
      ...
      self.handleError('this.s3.listUploads', 'Bucket ' + Bucket + ', contains no active multipart uploads');
    }
...
MultipartUploader.prototype.completeUpload = function(params, PartMap) {
  ...
  this.s3.completeMultipartUpload({
    ...
    // Get Information & Emit AWS Multipart Upload Complete
    self.getObjectInfo(completedata.Bucket, completedata.Key, function(err, infodata) {
      if(err) self.handleError('this.completeUpload.self.s3.completeMultipartUpload', err);
      self.emit('complete', { complete: completedata, info: infodata });
    });
      ...
      self.s3.createMultipartUpload(params, function(err, data) {
        if(err) {
          ...
          ws.on('error', function(err) {
            self.handleError('createMultipartUpload:ws:Writable', err);
          });
