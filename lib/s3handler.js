          ...
          if (!part.filename) {
            ...
            if(options.testmode === false) {
              ...
              */
              ...
              part.on('end', function(name, value) {
                formevent('test part.end',  {name: name, value: value});
                  async.parallel({
                   one: function(callback) {
                      middleware.emit('complete', { e: 'test:aws:complete', data: {} });
                      callback(null, 1);
                    },
                    two: function(callback) {
                      info.size = streamed;
                      info.finished = new Date();
                      info.extension = fileext(info.name);
                      info.key = options.aws.keyPrefix + ((options.aws.uuid) ? info.uuid : info.name);
                      info.bucket = options.aws.bucket;
                      info.location = 'https://'+options.aws.bucket+'.s3-'+options.aws.region+'.amazonaws.com/' + info.key.replace('/', '%2F');
                      info.prefix = options.aws.keyPrefix;
                      files.push(info);
                      callback(null, 2);
                    }
                  }, function(err, results) {
